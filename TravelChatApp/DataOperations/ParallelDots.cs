...
namespace ParallelDots
{
    ...
    {
        private static string  api_key= "gy74Q3hvgI9uYe0yHQGVBKk03AR5rVAkjKtwAXk4gHE";
        public static string host = "https://apis.paralleldots.com/v4/";
        ...
        public static string GetSentiment(JArray text)
        {
            ...
            double positive;
            Double.TryParse(json.Split("positive")[1].Split(":")[1].Split("}")[0], out positive);

            double neutral;
            Double.TryParse(json.Split("neutral")[1].Split(":")[1].Split(",")[0], out neutral);
        ...
        public string emotion(string text, string lang_code)
        {
            if ( api_key != null)
            {
                ...
                Console.WriteLine(text,  api_key);
                request.AddHeader("cache-control", "no-cache");
                //request.AddHeader("Content-Type", "multipart/form-data");
                request.AddHeader("source", "c#wrapper");
                IRestResponse response = client.Execute(request);
